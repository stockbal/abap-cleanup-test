"! <p class="shorttext synchronized">Factory for favorite menu</p>
CLASS zcl_dbbr_favmenu_factory DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Clear most used list</p>
    METHODS clear_most_used_list.

    "! <p class="shorttext synchronized">Delete favorite</p>
    METHODS delete_favorite
      IMPORTING
        is_favmenu TYPE zdbbr_favmenu.

    "! <p class="shorttext synchronized">Delete favorite entries for query name range</p>
    METHODS delete_favs_for_query_names
      IMPORTING
        it_query_name_range TYPE zif_sat_ty_global=>ty_t_selopt.

    "! <p class="shorttext synchronized">Delete global favorites</p>
    METHODS delete_global_favorites.

    "! <p class="shorttext synchronized">Delete most used entry</p>
    METHODS delete_most_used
      IMPORTING
        iv_entry TYPE zdbbr_favmenu-fav_entry
        iv_type  TYPE zdbbr_favmenu-favtype.

    "! <p class="shorttext synchronized">Delete multiple most used entries</p>
    "!
    "! @parameter it_most_used_k | <p class="shorttext synchronized"></p>
    METHODS delete_most_used_multiple
      IMPORTING
        it_most_used_k TYPE tt_mostused_k.

    "! <p class="shorttext synchronized">Delete user favorite</p>
    METHODS delete_user_favorites.

    "! <p class="shorttext synchronized">Check if given favorite exists</p>
    ...
    "! <p class="shorttext synchronized">Retrieve Favorites</p>
    ...
    "! <p class="shorttext synchronized">Get most used favorites</p>
    ...
    "! <p class="shorttext synchronized">Get next object id</p>
    ...
    "! <p class="shorttext synchronized">Checks if global favorite exists</p>
    METHODS global_favorites_exist
      RETURNING
        VALUE(rf_global_favs_exists) TYPE boolean.

    "! <p class="shorttext synchronized">Refresh most used entries</p>
    ...
    "! <p class="shorttext synchronized">Repair favorites tree</p>
    METHODS repair_favorite_tree.

    "! <p class="shorttext synchronized">Update description of favorite</p>
    METHODS update_favorite
      IMPORTING
        is_favmenu TYPE zdbbr_favmenu.

    "! <p class="shorttext synchronized">Update multiple favorites</p>
    METHODS update_favorites
      IMPORTING
        it_favorites TYPE zdbbr_favmenu_itab.

    "! <p class="shorttext synchronized">Checks if user favorites exist</p>
  ...
  PRIVATE SECTION.
    ...
    TYPES tt_object_id_duplicate TYPE STANDARD TABLE OF ty_object_id_duplicate WITH DEFAULT KEY.

    "! <p class="shorttext synchronized">Check if tree repair is needed</p>
    ...
    "! <p class="shorttext synchronized">Update parent id</p>
...
CLASS zcl_dbbr_favmenu_factory IMPLEMENTATION.
  ...
  METHOD get_favorites.
    DATA(lv_username) = COND sy-uname( WHEN if_global_favorites = abap_true
                                       THEN ''
                                       ELSE sy-uname ).
  ...
  METHOD get_most_used_favorites.
    DATA(lv_last_used_count) = COND #(
      WHEN if_all = abap_true
      THEN 0
      ELSE zcl_dbbr_usersettings_factory=>get_last_used_count_setting( ) ).
  ...
  METHOD refresh_most_used.
    zcl_dbbr_appl_util=>get_current_datetime( IMPORTING ev_time = DATA(lv_time)
    ...
    IF sy-subrc <> 0.
      ls_most_used_entry = VALUE #( most_used_entry     = iv_entry
                                    ...
                                    text                = iv_text ).
    ELSE.
      ls_most_used_entry-counter = ls_most_used_entry-counter + 1.
    ENDIF.
  ...
  METHOD repair_favorite_tree.
    ...
    DATA lt_object_id_counts TYPE tt_object_id_duplicate.
    DATA lt_favmenu_delete TYPE STANDARD TABLE OF zdbbr_favmenu.
    DATA lt_favmenu_new TYPE STANDARD TABLE OF zdbbr_favmenu.
    DATA lt_favmenu_all TYPE STANDARD TABLE OF zdbbr_favmenu.
    ...
    LOOP AT lt_favmenu_all ASSIGNING FIELD-SYMBOL(<ls_favmenu>)
      ...
      LOOP AT GROUP <ls_favmenu_group> ASSIGNING FIELD-SYMBOL(<ls_favmenu_group_entry>).
        ...
        TRY.
            DATA(ls_object_id_duplicate) = lt_object_id_counts[ uname     = <ls_favmenu_group_entry>-uname
                                                                object_id = <ls_favmenu_group_entry>-object_id ].

            DATA(ls_favmenu_new) = <ls_favmenu_group_entry>.
            lr_max_object_id->max_object_id = lr_max_object_id->max_object_id + 1.
            ...
            ls_object_id_duplicate-updated_object_id_count = ls_object_id_duplicate-updated_object_id_count + 1.
    ...
    LOOP AT lt_folder_object_ids ASSIGNING FIELD-SYMBOL(<ls_folder_id>).
      update_parent_id_local( EXPORTING iv_new_parent_id   = <ls_folder_id>-new_object_id
                                        iv_old_parent_id   = <ls_folder_id>-old_object_id
                              CHANGING  ct_favmenu         = lt_favmenu_all
