...
CLASS zcl_dbbr_query_copier IMPLEMENTATION.
  ...
  METHOD copy_query.
    ...
    DATA(ls_existing_query) = mr_query_f->get_query( iv_query_name = mv_trg_query_name ).
    ...
    DATA(ls_query_data) = mr_query_f->get_query_by_id( iv_query_id               = mv_src_query_id
    ...
    fill_id_values( EXPORTING iv_new_query_id = lv_new_query_id
  ...
  METHOD validate_query.
    ...
    IF mv_src_query_name = mv_trg_query_name.
      RAISE EXCEPTION TYPE zcx_dbbr_exception
        EXPORTING textid = zcx_dbbr_exception=>query_self_copy_impossible
    ...
    IF ls_existing_query IS NOT INITIAL.
      ...
      IF ls_existing_query-created_by <> sy-uname.
        RAISE EXCEPTION TYPE zcx_dbbr_exception
          EXPORTING textid = zcx_dbbr_exception=>query_overwrite_not_possible
