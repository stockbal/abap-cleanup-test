...
CLASS zcl_dbbr_package_factory IMPLEMENTATION.
  METHOD get_package.
    cl_package_factory=>load_package( EXPORTING  i_package_name             = to_upper( iv_package )
*                                                 i_force_reload             = " Force System to Read from Database (X=Yes)
                                      IMPORTING  e_package                  = rr_package    " Package Instance
                                      EXCEPTIONS object_not_existing        = 1
                                                 unexpected_error           = 2
                                                 intern_err                 = 3
                                                 no_access                  = 4
  ...
  METHOD get_packages_by_appl_hierarchy.
    ...
    DATA lt_app_comp TYPE HASHED TABLE OF ty_s_app_comp WITH UNIQUE KEY node_key.
    DATA lr_app_component TYPE REF TO zcl_dbbr_application_component.

    DATA(lr_app_comp_list) = CAST zif_uitb_list( NEW zcl_uitb_object_list( ) ).

    CALL FUNCTION 'RS_COMPONENT_VIEW'
      EXPORTING object_type = 'HF'
*                refresh     =
*                without_tree_setting =
*                ignore_sfw_switches = SPACE
*                refresh_all = SPACE
      TABLES    nodetab     = lt_nodes.
    ...
    LOOP AT lt_nodes ASSIGNING FIELD-SYMBOL(<ls_node>) FROM 2.
      IF <ls_node>-type = 'HF'.
        ...
        IF <ls_node>-tlevel = 2.
          lr_app_component = NEW zcl_dbbr_application_component( iv_app_component      = CONV #( <ls_node>-name )
      ...
      ELSEIF <ls_node>-type = 'DEVC'.
        lr_app_component->add_package( iv_package_name = <ls_node>-name
