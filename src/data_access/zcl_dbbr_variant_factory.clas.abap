"! <p class="shorttext synchronized">Factory for variant accesses</p>
CLASS zcl_dbbr_variant_factory DEFINITION
  ...
  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Checks if the variant exists</p>
    ...
    "! <p class="shorttext synchronized">Finds variants for several criteria</p>
    ...
    "! <p class="shorttext synchronized">Finds variants for the given query id</p>
    ...
    "! <p class="shorttext synchronized">Finds variant information for the given query id</p>
    ...
    "! <p class="shorttext synchronized">Finds variant information for the given entity/type</p>
    ...
    "! <p class="shorttext synchronized">Saves the given variant</p>
    CLASS-METHODS save_variant
      IMPORTING
        is_var_data TYPE zdbbr_variant_data.

    "! <p class="shorttext synchronized">Saves the default variant of an entity</p>
    CLASS-METHODS save_default_variant
      IMPORTING
        is_var_data TYPE zdbbr_variant_data.

    "! <p class="shorttext synchronized">Delete the default variant of an entity</p>
    ...
    "! <p class="shorttext synchronized">Saves the auto variant of an entity</p>
    CLASS-METHODS save_auto_variant
      IMPORTING
        is_var_data TYPE zdbbr_variant_data.

    "! <p class="shorttext synchronized">Deletes an auto variant of an entity</p>
    CLASS-METHODS delete_auto_variant
      IMPORTING
        iv_entity_id   TYPE zsat_entity_id
        iv_entity_type TYPE zsat_entity_type.

    "! <p class="shorttext synchronized">Saves a list of variants</p>
    CLASS-METHODS save_variants
      IMPORTING
        it_variants TYPE zdbbr_variant_data_itab.

    "! <p class="shorttext synchronized">Retrieves a variant with all its stored information</p>
    ...
    "! <p class="shorttext synchronized">Retrieves automatically saved variant for the given entity</p>
    ...
    "! <p class="shorttext synchronized">Retrieves default variant for the given entity</p>
    ...
    "! <p class="shorttext synchronized">Finds the default variant of a query</p>
    ...
    "! <p class="shorttext synchronized">Deletes the variant for the given id</p>
    CLASS-METHODS delete_variant
      IMPORTING
        iv_variant_id TYPE zdbbr_variant_id.

    "! <p class="shorttext synchronized">Deletes variants for the given query id</p>
    CLASS-METHODS delete_variants_by_query_id
      IMPORTING
        iv_query_id TYPE zdbbr_query_id.

    "! <p class="shorttext synchronized">Checks if there is variant for the given entity</p>
    ...
    "! <p class="shorttext synchronized">Checks if there is a default variant for the given entity</p>
  ...
  PRIVATE SECTION.
    "! <p class="shorttext synchronized">Deletes dependent data of the given variant id</p>
    CLASS-METHODS delete_corresponding_vardata
      IMPORTING
        iv_variant_id TYPE zdbbr_variant_id.

    "! <p class="shorttext synchronized">Loads dependent data of the passed variant</p>
...
*&---------------------------------------------------------------------*
  ...
  METHOD find_variants.
    DATA(lt_entity_id_sel) = COND #( WHEN iv_entity_id IS NOT INITIAL
                                     THEN VALUE zif_sat_ty_global=>ty_t_selopt( ( sign = 'I' option = 'EQ' low = iv_entity_id ) ) ).
    DATA(lt_entity_type_sel) = COND #( WHEN iv_entity_type IS NOT INITIAL
                                       THEN VALUE zif_sat_ty_global=>ty_t_selopt( ( sign = 'I' option = 'EQ' low = iv_entity_type ) ) ).
  ...
  METHOD find_variant_infos_for_type.
    ...
    IF iv_entity_type = zif_sat_c_entity_type=>query.
      ...
      IF lt_queries IS INITIAL OR lines( lt_queries ) <> 1.
        RETURN.
      ENDIF.
  ...
  METHOD get_variant.
    ...
    ELSE. " read variant for business key
      ...
      DATA(lt_entity_type_sel) = COND #( WHEN iv_entity_type IS NOT INITIAL
                                         THEN VALUE zif_sat_ty_global=>ty_t_selopt( ( sign = 'I' option = 'EQ' low = iv_entity_type ) ) ).
      DATA(lt_variant_name_selopt) = COND #( WHEN iv_variant_name IS NOT INITIAL
                                             THEN VALUE zif_sat_ty_global=>ty_t_selopt(
                                                            ( sign = 'I' option = 'EQ' low = iv_variant_name ) ) ).
  ...
  METHOD save_default_variant.
    ...
    DATA(ls_existing_var) = get_default_variant( iv_entity_id       = is_var_data-entity_id
  ...
  METHOD save_auto_variant.
    ...
    delete_auto_variant( iv_entity_id   = is_var_data-entity_id
                         iv_entity_type = is_var_data-entity_type ).
...
*&---------------------------------------------------------------------*
  ...
  METHOD variant_exists_for_entity.
    ...
    IF iv_entity_type = zif_sat_c_entity_type=>query.
      ...
      DATA(ls_query) = lr_query_f->get_query( iv_query_name      = iv_entity_id
                                              if_load_completely = abap_false ).
