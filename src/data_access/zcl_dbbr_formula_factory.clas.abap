...
CLASS zcl_dbbr_formula_factory IMPLEMENTATION.
  METHOD get_formulas.
    ...
    lt_created_by_range = COND #(
      WHEN iv_created_by IS NOT INITIAL
      THEN VALUE #( ( sign = 'I' option = 'EQ' low = iv_created_by ) )
      ELSE VALUE #( ( sign = 'E' option = 'EQ' low = sy-uname ) ) ).
  ...
  METHOD save_formula_def.
    DATA(ls_formula) = VALUE zdbbr_ffdef( id             = zcl_sat_system_helper=>create_guid_22( )
