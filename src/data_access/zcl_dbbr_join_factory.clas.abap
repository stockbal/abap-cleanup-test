CLASS zcl_dbbr_join_factory DEFINITION
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Fill missing alias names and entity types</p>
    "!
    METHODS repair_join
      CHANGING
...
CLASS zcl_dbbr_join_factory IMPLEMENTATION.
  ...
  METHOD read_join.
    ...
    LOOP AT lt_tables ASSIGNING FIELD-SYMBOL(<ls_table>).
      ...
      ls_join_table-field_conditions  = VALUE #( FOR field IN lt_field_cond
                                                 WHERE ( ref_join_table_id = <ls_table>-join_table_id )
                                                 ( field ) ).
      " .... Find filter conditions for this join table
      ls_join_table-filter_conditions = VALUE #( FOR filter IN lt_filter_cond
                                                 WHERE ( ref_join_table_id = <ls_table>-join_table_id )
                                                 ( filter ) ).
  ...
  METHOD save_join.
    ...
    DATA lt_tables TYPE STANDARD TABLE OF zdbbr_joint.
    DATA lt_field_cond TYPE STANDARD TABLE OF zdbbr_joinfld.
    DATA lt_filter_cond TYPE STANDARD TABLE OF zdbbr_joinfil.
  ...
  METHOD repair_join.
    ...
    IF cs_join-primary_table_alias IS NOT INITIAL.
      RETURN.
    ENDIF.
