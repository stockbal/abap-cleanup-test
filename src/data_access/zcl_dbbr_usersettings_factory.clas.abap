"! <p class="shorttext synchronized">Factory for user settings</p>
CLASS zcl_dbbr_usersettings_factory DEFINITION
  ...
  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Retrieves the current settings of the user</p>
    CLASS-METHODS get_current_settings
      RETURNING
        VALUE(rr_s_settings) TYPE REF TO zdbbr_user_settings_a.

    "! <p class="shorttext synchronized">Retrieve Settings for entity browser</p>
    CLASS-METHODS get_entity_browser_settings
      RETURNING
        VALUE(rs_settings) TYPE zdbbr_entbrwsus.

    "! <p class="shorttext synchronized">Save Settings for Entity Browser</p>
...
CLASS zcl_dbbr_usersettings_factory IMPLEMENTATION.
  METHOD get_entity_browser_settings.
    ...
    IF sy-subrc <> 0.
      rs_settings = VALUE #( max_hits        = 500
  ...
  METHOD get_settings.
    ...
    result = VALUE #( color_sort_columns          = abap_true
