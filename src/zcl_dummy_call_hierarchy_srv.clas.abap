...
CLASS zcl_dummy_call_hierarchy_srv DEFINITION
  ...
  PRIVATE SECTION.
    ...
    CLASS-DATA:
      instance TYPE REF TO zif_dummy_call_hierarchy_srv.

    DATA:
      factory           TYPE REF TO zif_dummy_abap_element_fac,
      ...
      descr_reader      TYPE REF TO zif_dummy_elem_descr_reader,
      current_element   TYPE REF TO zif_dummy_abap_element.
...
CLASS zcl_dummy_call_hierarchy_srv IMPLEMENTATION.
  ...
  METHOD create_abap_element.
    ...
    IF direct_ref-tag = cl_abap_compiler=>tag_method.
      new_elem_info-method_props = zcl_dummy_method_info_reader=>get_instance( )->read_properties( full_name = full_name ).
  ...
  METHOD determine_correct_src_pos.
    ...
    IF abap_element_info-tag = cl_abap_compiler=>tag_method AND
        abap_element_info-method_props-encl_type = zif_dummy_c_tadir_type=>interface.
